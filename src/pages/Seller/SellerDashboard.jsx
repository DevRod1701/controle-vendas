...
const SellerDashboard = () => {
  ...
  const debt = useMemo(() => myOrders.reduce((acc, o) => {
    if (o.type === 'sale' && o.status === 'approved') return acc + (Number(o.total || 0) - Number(o.paid || 0));
    return acc;
  }, 0), [myOrders]);
  ...
  const currentMonthSales = useMemo(() => {
    ...
    return myOrders.reduce((acc, o) => {
      ...
      if (o.type === 'sale' && o.status === 'approved' && d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear()) return acc + Number(o.total || 0);
      return acc;
    }, 0);
  }, [myOrders]);
  ...
  const handleBulkPayment = async (paymentData) => {
     ...
     const unpaidOrders = myOrders.filter(o => o.status === 'approved' && o.type === 'sale' && (Number(o.total) - Number(o.paid || 0)) > 0.01).sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
     for (const order of unpaidOrders) {
        if (remaining <= 0) break;
        const currentDebt = Number(order.total) - Number(order.paid || 0);
        const payAmount = Math.min(remaining, currentDebt);
        await supabase.from('payments').insert([{ order_id: order.id, amount: payAmount, date: paymentData.date, method: paymentData.method, proof: paymentData.proof, description: 'Pagamento Agrupado' }]);
        const newPaid = Number(order.paid || 0) + payAmount;
        await supabase.from('orders').update({ paid: newPaid }).eq('id', order.id);
        remaining -= payAmount;
     }
     setShowPayModal(false); refreshData(); alert("Pagamento distribuído com sucesso!");
  };

  return (
    <div className="p-6 pb-24 space-y-6 animate-in slide-in-from-top-4 text-left font-bold">
      {showPayModal && <BulkPaymentModal totalDebt={debt} onClose={() => setShowPayModal(false)} onConfirm={handleBulkPayment} />}
      ...
      <div className="bg-slate-900 p-8 rounded-[3.5rem] text-white space-y-4 shadow-xl relative overflow-hidden border-b-8 border-yellow-400">
        ...
        <div className="flex gap-2 pt-2">
            ...
            <button onClick={() => setShowPayModal(true)} className="flex-1 py-4 bg-yellow-400 text-slate-900 rounded-2xl font-black text-xs uppercase active:scale-95 shadow-lg">Pagar Pedido</button>
        </div>
      </div>
      ...
      <button onClick={() => navigate('/historico')} className="w-full bg-green-50 p-6 rounded-[2.5rem] border-2 border-green-100 flex items-center justify-between shadow-sm active:scale-95 transition-all text-left">
        <div><p className="text-[10px] font-black text-green-600 uppercase tracking-widest">Vendas Aprovadas (Mês)</p><p className="text-2xl font-black text-green-700 font-mono tracking-tighter">{formatBRL(currentMonthSales)}</p></div>
