...
import { ArrowLeft, Upload, DollarSign, Undo2, ImageIcon, AlertCircle, Loader2 } from 'lucide-react';
// IMPORTANTE: Caminhos ajustados para estar em src/components/
import { supabase } from '../services/supabase';
import { useAuth } from '../contexts/AuthContext';
import { formatBRL } from '../utils/formatters';
import AlertModal from './modals/AlertModal'; // Agora busca na pasta vizinha 'modals'
...
const OrderDetail = ({ order, onClose, refreshData }) => {
  ...
  // TRAVA DE SEGURANÇA (Double Submit)
  const [isSubmitting, setIsSubmitting] = useState(false); 
  
  // ESTADO PARA ALERTAS (Substitui window.alert)
  ...
  // BLOQUEIO: Só pode pagar se estiver APROVADO
  ...
  if (isPaying) {
    return (
      <div className="fixed inset-0 bg-white z-[300] p-6 animate-in slide-in-from-bottom-10 overflow-y-auto font-bold">
        {renderAlert()}
        <div className="flex items-center gap-3 mb-6"><button onClick={() => setIsPaying(false)} className="p-3 bg-slate-100 rounded-2xl"><ArrowLeft size={20}/></button><h2 className="text-xl font-black text-slate-800 uppercase">Pagar</h2></div>
        <div className="space-y-4">
          <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 text-center"><p className="text-xs text-slate-400 font-bold uppercase">Restante</p><p className="text-3xl font-black text-slate-800 font-mono">{formatBRL(pendente)}</p></div>
          ...
          <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase ml-2">Descrição (Opcional)</label><input type="text" className="w-full p-4 bg-slate-50 rounded-2xl border-none font-bold text-sm outline-none" value={paymentData.desc} onChange={e => setPaymentData({...paymentData, desc: e.target.value})} placeholder="Ex: Acerto semanal..." /></div>
          ...
          <div className="space-y-1">
             ...
             {paymentData.proof && <p className="text-[10px] text-green-600 font-bold text-center">Imagem carregada!</p>}
          </div>

          <button 
             onClick={handlePay} 
             disabled={isSubmitting} 
             className="w-full py-4 bg-green-500 text-white rounded-2xl font-bold uppercase shadow-lg mt-4 disabled:opacity-50 flex items-center justify-center gap-2"
          >
  ...
  if (isReturning) {
    return (
      <div className="fixed inset-0 bg-white z-[300] p-6 animate-in slide-in-from-bottom-10 overflow-y-auto font-bold">
        {renderAlert()}
        <div className="flex items-center gap-3 mb-6"><button onClick={() => setIsReturning(false)} className="p-3 bg-slate-100 rounded-2xl"><ArrowLeft size={20}/></button><h2 className="text-xl font-black text-slate-800 uppercase">Devolver</h2></div>
        
        {order.original_order_id && <div className="bg-orange-50 p-4 rounded-2xl mb-4 border border-orange-100"><p className="text-[10px] text-orange-400 font-bold uppercase">Devolvendo do Pedido</p><p className="text-orange-700 font-mono font-bold">#{order.id.slice(0,5)}</p></div>}

        <p className="text-xs text-slate-400 font-bold mb-4 uppercase">Selecione o que sobrou:</p>
        <div className="space-y-3">
          {order.order_items.filter(i => i.qty > 0).map(item => (
            <div key={item.id} className="bg-slate-50 p-4 rounded-2xl flex justify-between items-center border border-slate-100">
              <div><p className="font-bold text-slate-800">{item.name}</p><p className="text-xs text-slate-400">Levou: {item.qty}</p></div>
        ...
        <button 
            ...
            disabled={isSubmitting} 
            className="w-full py-4 bg-orange-500 text-white rounded-2xl font-bold uppercase shadow-lg mt-6 disabled:opacity-50 flex items-center justify-center gap-2"
        >
    ...
    <div className="fixed inset-0 bg-white z-[150] animate-in slide-in-from-right overflow-y-auto font-bold">
      ...
      <div className="p-6 pb-24 space-y-6">
        ...
        <div>
          ...
          <div className="space-y-2">
            {order.order_items.map((item, i) => { 
                ...
                return (<div key={i} className="flex justify-between p-4 bg-white border border-slate-100 rounded-2xl shadow-sm"><span className="font-bold text-slate-700">{item.qty}x {item.name}</span><span className="font-mono text-slate-500">{formatBRL(item.price * item.qty)}</span></div>); 
            })}
            
            {order.order_items.map((item, i) => { 
                if (item.qty > 0) return null; 
                return (<div key={'zero-'+i} className="flex justify-between p-4 bg-red-50 border border-red-100 rounded-2xl opacity-70"><span className="font-bold text-red-400 line-through">Devolvido: {item.name}</span><span className="font-mono text-red-400 line-through">{formatBRL(item.price * item.qty)}</span></div>); 
            })}
            
            {returnedItemsHistory.length > 0 && (
                <div className="pt-2">
                    <div className="flex items-center gap-2 mb-2 mt-2"><AlertCircle size={12} className="text-red-400"/><p className="text-[10px] font-bold text-red-400 uppercase">Histórico de Devoluções</p></div>
                    {returnedItemsHistory.map((item, i) => (<div key={'hist-'+i} className="flex justify-between p-4 bg-red-50 border border-red-100 rounded-2xl mb-2 opacity-70"><span className="font-bold text-red-400 line-through">Devolvido: {item.qty}x {item.name}</span><span className="font-mono text-red-400 line-through">{formatBRL(item.price * item.qty)}</span></div>))}
                </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-white border-t border-slate-100 flex flex-col gap-3">
          {/* BOTÃO DE PAGAR SÓ APARECE SE TIVER DÍVIDA E ESTIVER APROVADO */}
